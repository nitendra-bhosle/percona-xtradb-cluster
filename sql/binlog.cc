...
int MYSQL_BIN_LOG::gtid_end_transaction(THD *thd)
{
  ...
  {
    assert(thd->variables.gtid_next.type == GTID_GROUP);
...
#endif
   bytes_written(0), binlog_space_total(0), file_id(1),
   open_count(1), sync_period_ptr(sync_period), sync_counter(0),
                       ...
                       bool verify_checksum, bool is_relay_log)
{
  ...
  */
  if (all_gtids)
    assert(all_gtids->get_sid_map() == sid_map);
  if (prev_gtids)
...
int MYSQL_BIN_LOG::purge_first_log(Relay_log_info* rli, bool included)
{
  ...
  */
  ...
  assert(!included || rli->linfo.index_file_start_offset == 0);
      ...
      if (my_errno() == ENOENT) {
        ...
        */
        ...
        if (thd) {
          ...
          sql_print_information("Failed to stat log file '%s'",
                                log_info.log_file_name);
        }
        error = LOG_INFO_FATAL;
        goto done;
...
bool MYSQL_BIN_LOG::after_append_to_relay_log(Master_info *mi)
{
  ...
  assert(is_relay_log);
  assert(current_thd->system_thread == SYSTEM_THREAD_SLAVE_IO);
    ...
    if (buf_len == 0)
    {
      ...
      if (buf_len == 0)
      {
        /**
          ...
          and truncate the binlog.  /Sven
        */
        assert(my_b_tell(cache) == expected_total_len);
  ...
  else
  {
    ...
    */
    if (!thd->has_gtid_consistency_violation)
    {
      ...
      {
        assert(gtid_next_type == ANONYMOUS_GROUP);

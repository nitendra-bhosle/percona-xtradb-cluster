...
no_commit:
	...
	if (wsrep_on(m_user_thd) && wsrep_load_data_splitting &&
		...
		/* LDI that qualifies for intermediate commit logic.
		- Being LDI it suggest we are loading a table from
                  external file.
                - Check above also enforces that this LDI is not part of the
                  existing transaction.
                Based on above 2 conditions we can safely assume that the
                transaction will touch only table to load. If table to load is
                partitioned then main-table could be split into N sub-tables.
                Same needs to be considered. */

		std::vector<dict_table_t *> locked_table;
		populate_locked_table_list(m_prebuilt->trx, locked_table);

		WSREP_DEBUG(
		    "Forcing split of large transaction at %u rows "
		    " for LOAD DATA INFILE: %s",
		    m_num_write_row, wsrep_thd_query(m_user_thd));
		...
		/* this will avoid rotation of the binlog during
		...
		for (uint i = 0; i < locked_table.size(); ++i) {
			/* Re-acquire the table lock on the source table. */
...
report_error:
	...
	if (!error_result
		...
		if (wsrep_append_keys(m_user_thd, WSREP_KEY_EXCLUSIVE, record, NULL)) {
...
func_exit:
	...
	if (!err
		...
		if (wsrep_append_keys(m_user_thd, WSREP_KEY_EXCLUSIVE, old_row, new_row)) {
			DBUG_PRINT("wsrep", ("row key failed"));
	...
	if (wsrep_protocol_version == 0) {
		...
		for (i=0; i<table->s->keys; ++i) {
			...
			if ((!hasPK && wsrep_certify_nonPK) ||
				...
				{
					WSREP_DEBUG("Skip appending NULL key: %s",
						    wsrep_thd_query(thd));
				}
				if (record1) {
...
#ifdef WITH_WSREP
	    && existing_redo_encryption_mode != static_cast<redo_log_encrypt_enum>(target)) {
#else
...
wsrep_signal_replicator(trx_t *victim_trx, trx_t *bf_trx)
{
	...
	case QUERY_COMMITTING:
		...
		WSREP_DEBUG("Killing Transaction (%llu) in QUERY_COMMITTING"
			" state", (long long)victim_trx->id);
                            ...
                            trx_t *victim_trx, ibool signal)
{
	...
	case QUERY_EXEC:
	{
		...
		if (victim_trx->lock.wait_lock) {
			WSREP_DEBUG("victim has wait flag: %u",
	...
	case QUERY_IDLE:
	{
		...
		WSREP_DEBUG("Killing Transaction (%llu) in QUERY_IDLE state",
			(long long)victim_trx->id);
		...
		while (abortees && !skip_abort) {
			...
			if (abortees->aborting_thd == thd) {
				skip_abort = true;
				WSREP_WARN("Duplicate thd aborter %u",
					  wsrep_thd_thread_id(thd));
			}
		...
		if (!skip_abort) {
			...
			wsrep_aborting_thd      = aborting;
 			DBUG_PRINT("wsrep",("enqueuing trx abort for %u",
                                             wsrep_thd_thread_id(thd)));
			WSREP_DEBUG("Enqueue transaction for force-abort %u",
				    wsrep_thd_thread_id(thd));
		}

		DBUG_PRINT("wsrep",("signalling wsrep rollbacker"));
		WSREP_DEBUG("Signal Rollback/Aborter");
